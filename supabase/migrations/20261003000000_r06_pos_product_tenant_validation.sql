-- R06 (2026-09-23 register: POS sale command must never create negative
-- stock, post atomic stock+financial effects, reject cross-tenant product
-- references, and keep client input from bypassing server stock rules).
--
-- This is the ONLY code change of the 2026-09-23 R06 verification package;
-- everything else it mandates was already structurally present and is proven
-- by the accompanying release suite (tests/release/r06-stock.test.ts).
--
-- DEMONSTRATED DEFECT (why a migration is genuinely required):
--   * probes at the prior tip: a POS sale of business A whose lines carry
--     business B's product id COMMITTED — invoice, payments, sales/settlement
--     journals all posted — while the stock release silently skipped the
--     foreign product (tenant-scoped lookup miss in
--     _ledgr_complete_pos_sale). The result was a committed financial
--     document claiming an item was sold, with zero inventory effect and no
--     signal to the caller — the same divergence class as the original
--     POS.STOCK failure.
--   * The existing invariant cannot prevent it:
--     chk_inventory_balances_on_hand_nonneg constrains balances only, and the
--     movement→balance trigger never fires because no movement row is ever
--     inserted for a foreign product. Schema CHECK constraints cannot express
--     cross-table tenant validation; a FK on invoice_lines.product_id admits
--     any tenant's product and tightening it would constrain every
--     non-POS invoice writer (out of R06 scope).
--   * Minimal correction: validate caller-supplied product references inside
--     post_pos_sale itself — the POS command boundary this package owns —
--     before any write happens. One validation block, zero behavior change
--     for every legitimate payload (products are tenant-immutable, so a
--     previously-committed legitimate sale replays identically).
--
-- NO-WEAKENING PROOF: the change only RAISES (22023) earlier for a class of
-- payloads that previously committed a divergent half-document. It does not
-- touch: branch/terminal/shift gates (R08, byte-preserved below), tender
-- settlement math, idempotency/replay paths, the movement gate, the
-- movement→balance trigger, the 23514 non-negativity policy, quota (still
-- asserted after validation, before numbering), or the completion helper.
--
-- ROLLBACK: re-apply 20260930000001_r08_post_pos_sale_binding.sql (same
-- create-or-replace shape) to restore the prior function body exactly.
--
-- The function body below is a verbatim copy of
-- 20260930000001_r08_post_pos_sale_binding.sql with exactly one injected
-- validation block (comment marker "3b. R06"); validation discipline requires
-- a full harness rerun (tests/release, two consecutive deterministic runs)
-- which this package performs.
...
begin
  ...
  --     Verified defect: a sale whose lines carry another business's product
  --     id used to COMMIT with zero stock effect — the completion helper
  --     silently skipped the foreign product while the financial half posted
  --     (documented stock/financial divergence on a committed document).
  --     Denial is atomic at 22023, before any write, quota consumption or
  --     document-number reservation. Manual lines (no product) and the
  --     business's own service products (track_inventory=false) are
  --     unaffected; neither behavior is altered.
  if exists (
    select 1
