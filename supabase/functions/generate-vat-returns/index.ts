...
import { priorMonthPeriod, sumTaxAmounts, computeAmountDue, alertSchedule } from '../_shared/vatReturn.ts';
...
Deno.serve(async (req) => {
  ...
  // The shared pure module also fixes a latent off-by-one: the previous
  // toISOString pipeline rendered local midnights in UTC, shifting period
  // bounds/due date back a day in timezones ahead of UTC.
  const { periodStart: periodStartStr, periodEnd: periodEndStr, periodLabel, dueDate } = priorMonthPeriod(new Date());
  ...
  for (const business of businesses ?? []) {
    try {
      ...
      // shared `packages/tax-core` module Phase 2 onward. The arithmetic
      // itself (period bounds, tax summation, amount-due rounding, alert
      // schedule) now lives in ../_shared/vatReturn.ts, which vitest covers.
      ...
      if (expenseIds.length > 0) {
        ...
        inputTax = sumTaxAmounts(lines ?? []);
      }

      const amountDue = computeAmountDue(outputTax, inputTax);
      ...
      const alertRows = alertSchedule(dueDate).map((alert) => ({
        business_id: business.id,
