...
export function CookieConsentBanner() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white p-4 shadow-2xl sm:p-5">
        ...
        {!customizing ? (
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setCustomizing(false)} className="text-gray-400 hover:text-gray-600">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Required for sign-in and core functionality. Always on.</p>
                </div>
              ...
              <div className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Helps us understand how Ledgr is used, so we can improve it.</p>
                </div>
                <button
                  onClick={() => setAnalytics((v) => !v)}
                  className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${analytics ? 'bg-brand-500' : 'bg-gray-300'}`}
                >
              ...
              <div className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Used for tailored offers and communications.</p>
                </div>
                <button
                  onClick={() => setMarketing((v) => !v)}
                  className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${marketing ? 'bg-brand-500' : 'bg-gray-300'}`}
                >
                  <span className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${marketing ? 'translate-x-5' : 'translate-x-0.5'}`} />
                </button>
              </div>
            </div>
