...
export function NewJournalEntryModal({ businessId, onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-card shadow-xl">
        <div className="flex items-center justify-between border-b border-line px-6 py-4">
          <h2 className="text-base font-semibold text-ink">New Journal Entry</h2>
        ...
        <div className="space-y-4 px-6 py-5">
          ...
          {isDateLocked && (
            <div className="flex items-start gap-2 rounded-lg bg-warning/12 p-3 text-xs text-warning">
          ...
          <div className="rounded-xl border border-line">
            <div className="grid grid-cols-12 gap-2 border-b border-line bg-bg px-3 py-2 text-xs font-medium uppercase tracking-wide text-muted">
            ...
            {lines.map((line) => (
              <div key={line.id} className="grid grid-cols-12 items-center gap-2 border-b border-line px-3 py-2 last:border-b-0">
                <div className="col-span-5">
                  <select
                    value={line.accountId}
                    onChange={(e) => updateLine(line.id, { accountId: e.target.value })}
                    className="w-full rounded-lg border border-line px-2 py-1.5 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                  >
                ...
                <div className="col-span-2">
                  <input
                    ...
                    placeholder="0.00"
                    className="w-full rounded-lg border border-line px-2 py-1.5 text-right text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                  />
                </div>
                ...
                <div className="col-span-1 flex justify-end">
                  <button
                    ...
                    className="rounded p-1 text-muted/50 hover:bg-danger/10 hover:text-danger disabled:cursor-not-allowed disabled:opacity-30"
                  >
            ...
            <div className="px-3 py-2">
              <button
                onClick={addLine}
                className="flex items-center gap-1.5 text-xs font-medium text-brand-600 dark:text-brand-300 hover:text-brand-700 dark:text-brand-300"
              >
          ...
          {/* Balance summary */}
          <div className={`flex items-center justify-between rounded-lg px-4 py-3 text-sm ${isBalanced ? 'bg-brand-500/10 text-brand-700 dark:text-brand-300' : 'bg-bg text-sub'}`}>
          ...
          {submitError && (
            <div className="rounded-lg bg-danger/10 px-4 py-3 text-sm text-danger">{submitError}</div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-line px-6 py-4">
          <button
            onClick={onClose}
            className="rounded-lg px-4 py-2 text-sm font-medium text-sub hover:bg-bg"
          >
            Cancel
          </button>
          <button
