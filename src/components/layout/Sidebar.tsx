    ...
    <>
      ...
      >
        <div className="flex h-16 items-center gap-2 border-b border-line px-4">
          ...
          {sidebarOpen && (
            <span className="truncate text-lg font-semibold text-ink">{businessName}</span>
          )}
          {/* Mobile close button */}
          {sidebarOpen && (
            <button
              onClick={() => setSidebarOpen(false)}
              className="ml-auto rounded-lg p-1 text-muted hover:bg-brand-500/8 hover:text-ink lg:hidden"
            >
              <X className="h-5 w-5" />
            </button>
          )}
                    ...
                    <li key={item.path}>
                      ...
                      >
                        {({ isActive }) => (
                          <>
                            {isActive && (
                              <span className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-full bg-brand-600" aria-hidden />
                            )}
        ...
        <div className="border-t border-line p-3">
          <button
            ...
            className={clsx(
              'hidden lg:flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-sub transition-colors hover:bg-brand-500/8 hover:text-ink',
              !sidebarOpen && 'justify-center',
            )}
          >
            {sidebarOpen ? (
