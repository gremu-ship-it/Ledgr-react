...
import {
  ...
  syncAssetCapitalisation, backfillAssetCapitalisation,
  type DepreciationRunResult, type CapitalisationResult,
...
function AssetModal({
  existing, businessId, userId, categories, onClose,
}: {
  ...
  // Funding account for the capitalisation journal (DR asset / CR funding).
  ...
  const mutation = useMutation({
    mutationFn: async (): Promise<{ capWarning: string | null }> => {
      ...
      if (existing) {
        const { data, error } = await repos.asset['client']
          .from('fixed_assets').update(payload as never).eq('id', existing.id).select('*').single();
        if (error) throw new Error(error.message);
        saved = data as Row<'fixed_assets'>;
      } else {
        const { data, error } = await repos.asset['client']
          .from('fixed_assets').insert(payload as never).select('*').single();
        if (error) throw new Error(error.message);
        saved = data as Row<'fixed_assets'>;
      }

      // Post the capitalisation journal (DR asset account / CR funding
      // account) — or the cost-delta adjustment when editing an already
      // capitalised asset. Without this, the SOFP showed fixed assets at
      // zero because only the register row ever existed.
      try {
        await syncAssetCapitalisation(
          businessId,
          saved,
          existing ? existing.acquisition_cost : 0,
          form.funding_account_id,
          userId,
        );
        return { capWarning: null };
      } catch (e) {
        return { capWarning: (e as Error).message };
      }
    },
    onSuccess: (result) => {
      ...
      setAlert({
        type: 'success',
        message: result.capWarning
          ? `${existing ? 'Asset updated.' : 'Asset added.'} Capitalisation journal failed — fix the GL/funding accounts, then use "Capitalise assets": ${result.capWarning}`
      ...
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-xl">
        ...
        <div className="max-h-[72vh] overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Paid From (Funding Account) *</label>
              <select value={form.funding_account_id} onChange={(e) => set('funding_account_id', e.target.value)}
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Posts DR asset account / CR this account (bank, creditor or capital) so the asset appears on the Statement of Financial Position.
              </p>
            </div>
            <div>
...
// ── Capitalise Assets Modal (backfill) ────────────────────────────────────────

function CapitaliseAssetsModal({ businessId, userId, onClose }: {
  businessId: string; userId: string; onClose: () => void;
}) {
  const queryClient = useQueryClient();
  const [fundingAccountId, setFundingAccountId] = useState('');
  const [results, setResults] = useState<CapitalisationResult[] | null>(null);
  ...
  const mutation = useMutation({
    mutationFn: () => backfillAssetCapitalisation(businessId, fundingAccountId, userId),
    onSuccess: (res) => {
      setResults(res);
      queryClient.invalidateQueries({ queryKey: ['journal'] });
      queryClient.invalidateQueries({ queryKey: ['sofp'] });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <h3 className="text-base font-semibold text-gray-900">Capitalise assets to the ledger</h3>
        {!results ? (
          <>
            ...
            <div className="mt-4">
              <label className="mb-1 block text-sm font-medium text-gray-700">
                Assets were paid from (funding account) *
              </label>
              ...
              <p className="mt-1 text-xs text-gray-400">
                Typically a bank account, Cash on Hand, Trade Creditors (bought on credit), or
                Share Capital / Owner Drawings (brought in by the owner).
              </p>
            </div>
            {mutation.isError && (
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button onClick={() => mutation.mutate()} disabled={!fundingAccountId || mutation.isPending}
                className="rounded-lg bg-brand-500 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-600 disabled:opacity-60 transition-colors">
                {mutation.isPending ? 'Posting…' : 'Post capitalisation'}
              </button>
            </div>
          </>
        ) : (
          <>
            <div className="mt-3 max-h-64 overflow-y-auto space-y-1.5">
              ...
              {(results?.filter((r) => r.skipped) ?? []).map((r) => (
                <div key={r.assetId} className="flex items-start gap-2 rounded-lg bg-gray-50 px-3 py-2 text-xs">
                  <AlertCircle className="mt-0.5 h-3.5 w-3.5 text-gray-400 shrink-0" />
                  <span className="text-gray-600">{r.assetName}: {r.skipped}</span>
                </div>
              ))}
            </div>
            <p className="mt-3 text-xs text-gray-500">
              {posted.length} capitalisation journal{posted.length === 1 ? '' : 's'} posted.
              Refresh the Statement of Financial Position to see them.
            </p>
...
function AssetRegisterTab({ businessId, userId }: { businessId: string; userId: string }) {
  ...
  const [showCapitalise, setShowCapitalise] = useState(false);
    ...
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowCapitalise(true)}
            ...
            <Landmark className="h-4 w-4" />Capitalise assets
          </button>
      ...
      {showCapitalise && (
        <CapitaliseAssetsModal
          businessId={businessId}
          userId={userId}
          onClose={() => setShowCapitalise(false)}
        />
      )}
      {showRunDepreciation && (
