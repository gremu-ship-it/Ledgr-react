...
function parseMT940(text: string): ParsedStatement {
  ...
  for (const chunk of chunks) { const line = chunk.split(/\r?\n/)[0]; const m = line.match(/^:61:(\d{6})(?:\d{4})?([CD])(?:R?)([0-9,]+)/); if (!m) continue; const desc = chunk.match(/:86:([\s\S]*?)(?=\r?\n:\d{2}|$)/)?.[1].replace(/\r?\n/g, ' ').trim() || 'Bank transaction'; transactions.push({ date: `20${m[1].slice(0,2)}-${m[1].slice(2,4)}-${m[1].slice(4,6)}`, amount: money(m[3]), type: m[2] === 'D' ? 'debit' : 'credit', description: desc, reference: desc.match(/(?:REF|NONREF|\/\/)\s*([\w/-]+)/i)?.[1] }); }
