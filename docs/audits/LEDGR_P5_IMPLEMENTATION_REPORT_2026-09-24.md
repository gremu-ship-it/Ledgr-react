...
**Packages:** P5-A (Model 3) **PASS** + P5-B (Model 4 freeze) **PASS** + P5-C (Uniform Quota + Dual Authority) **PASS** + P5-D (DEC-03 Branch Scope / R08.7) **PASS** + P5-E (AI Branch Context / R11) **PASS** + P5-F (R09.4 Browser Verification) **PASS with documented BLOCKED/FAIL** — overall P5 **PARTIALLY COMPLETE** with honest BLOCKED (server revalidation) + measurement divergence (stale-version now quarantined per P5-A). This report is updated per package gate; see §7 (P5-E) and §8 (P5-F).
...
## §7 P5-E — AI.BRANCH + R11 (COMPLETE at 297337b — 807 unit PASS)

**Scope:** Signed DEC-03 + Q14 B / Q15 B (AI branch after P8) — `ai_context(business_id, branch_id?)` optional, `can_access_branch()` authorized, branch-filtered `WHERE branch_id` server-side, `R11` branch metric consistency (AI branch = authoritative reporting). No default on two-arg `ai_context(uuid,uuid)` (avoids 42725 ambiguity), wrapper `ai_context(uuid)`→`ai_context(uuid,null)`, hybrid `can_access_branch` (branch_manager/sales_manager NULL=org-wide, other assigned NULL fail-closed 42501), all `v_ai_*` branch/business-wide unions, revoke/grant both signatures.

### Implementation

- **Migration `20261008000000_p5e_ai_branch_context.sql` (additive, idempotent):**
  - Branch-aware views `v_ai_revenue_invoices`, `v_ai_expense_docs`, `v_ai_cash_accounts`, `v_ai_cash_movements`, `v_ai_kpis`, `v_ai_monthly_trend`, `v_ai_top_expenses`, `v_ai_top_customers`, `v_ai_customer_concentration`, `v_ai_upcoming_receivables`, `v_ai_upcoming_payables`, `v_ai_anomalies`, `v_ai_overdue_invoices` now expose `branch_id` (or `branch_id is null` for business-wide). `v_ai_kpis` / `v_ai_monthly_trend` / `v_ai_top_*` etc. implemented as `business-wide UNION ALL per-branch` with `is not distinct from v_effective_branch_id` filtering.
  - `ai_context(p_business_id uuid, p_branch_id uuid)` **no DEFAULT** — `SECURITY DEFINER` `stable` with R03 guards (auth, `is_business_member`, `v_reports_roles` 12 roles), DEC-03 branch authorization: service_role tenant-bound `branches` check, authenticated `can_access_branch` on supplied branch, omitted branch: assigned-scope with `branch_id` filters to own branch, assigned `NULL` fail-closed `42501` except hybrid `branch_manager`/`sales_manager` (reports-tier + assigned) where `NULL` remains org-wide, org-wide roles remain `null`. `v_effective_branch_id` applied via `WHERE branch_id is not distinct from v_effective_branch_id` at SQL layer (prompt hints never broaden). Revoke `public,anon`, grant `authenticated,service_role`, comment records DEC-03+R11.
  - One-arg compatibility: `ai_context(p_business_id uuid) RETURNS jsonb LANGUAGE sql` wrapper `SELECT ai_context(p_business_id, null::uuid)` with identical privileges and comment.
  - `create view` uses `security_invoker=true` for RLS; `cash_business GROUP BY 1,2` bug fixed.
- **Client:**
  - `src/lib/ai/context.ts` — `fetchAiData(businessId, branchId?)` → `supabase.rpc('ai_context',{p_business_id,p_branch_id:branchId??null})` (explicit `null` for org-wide, never omit), `buildAssistantContext(userId,businessId,mode,branchId?)` includes `branchId` in `DataContext`, `normaliseAiData` preserves branch-filtered shape.
  - `src/components/ai/Assistant.tsx` — `selectedBranchId` prop/state + `queryKey ['assistant-context',mode,businessId,userId,branchId]` isolation (prevents cross-branch cache contamination), `fetchAiData(businessId, branchId)` call.
  - `src/lib/ai/types.ts` + `src/lib/ai/provider.ts` + `supabase/functions/ai-chat/index.ts` — `DataContext.branchId` + `buildDataContext(businessId,branchId)` + prompt branch scope injection (branch-filtered data labelled).
- **Release harness:**
  - `tests/release/r03-ai.test.ts` — `has_function_privilege` checks **both** signatures on `anon`/`authenticated`/`service_role`, `DEF-MIRRORS-ROLE-MODEL` extracts `v_reports_roles` array for excluded-role check, `ROLE-GATE-MATRIX` with `cashier+branch` fixtures, `INVOCATION-FROM-NON-EXCLUDED-USER` with service_role branch isolation.

### Tests — P5-E matrix (10 PASS, total 807)

`src/lib/ai/__tests__/branch.test.ts` — **10 tests** covers P5-E + R11:
1. `fetchAiData` passes `p_branch_id null` org-wide
2. passes explicit `branchId`
3. degrades to `null` on `42501` branch denied (never throws)
4. `buildAssistantContext` includes `branchId` in `DataContext` + RPC args + company name fallback
5. support mode still carries `branchId`
6. `normaliseAiData` preserves branch-filtered shape (branch filtering server-side)
7. prompt hints never broaden access (server authority)
8. `assistant-context queryKey` includes `branchId` (isolation)
9. org-wide metrics equal sum of branch metrics (R11 coherence simulation)
10. natural language branch hints never bypass SQL filter

Existing `r03-ai 16/16 PASS`, full `src` **807 PASS / 0 FAIL** (91 files, +10 P5-E, up from 797), `tsc -b` clean, `eslint` 0/3w, `vite build` 1.97s PWA112, `test:release:types` clean.

### Gate result

- **P5-E PASS** — AI branch context with DEC-03 enforcement at `297337b`, both signatures `has_function_privilege` correct, wrapper + authorization + branch-filtered views coherent, cache isolation + R11 simulation proven, no product regressions.

## §8 P5-F — R09.4 Browser Verification (verification-only — service-worker / offline queue provenance / revalidation contract)

**Scope:** Verify existing R09.4 contract via **actual browser evidence only** (no simulation). Mandatory coverage per P5-F: service-worker registration/scope/version/cache (R09.1), cache wipe (signed-out / user-switch), IndexedDB queue provenance (`clientKey`/`payloadHash`/`branch`/`terminal`/`payloadVersion`/`operationType`) and identity isolation, P5-A quarantine (`stale-version`/`unknown-version`/`clientKey-payload-mismatch` durable not retried not reconcilable), P5-B reconcilable freeze (`stock-denied`/`policy-denied` only, `branch-denied`/`terminal-denied` typed `failed` not reconcilable), branch/terminal-denied typed failures, P5-C quota dual authority (`enqueue` `P0QLT` early guard fail-open on non-P0QLT vs `BEFORE INSERT`+`FOR UPDATE` authority), offline/online transitions (enqueue offline → online replay exactly once), lost-ack exactly-once `clientKey` idempotency, IDB tamper revalidation (`payload-tampered`), service-worker interception, multi-tab lease exclusivity, network evidence. **Do NOT claim browser verification from unit/static/TS/service-worker source alone; if infrastructure unavailable mark BLOCKED and document limitation.**

### Browser test environment (P5-F §2)

- **Driver:** `playwright-core@1.49.1` driving `@sparticuz/chromium@138.0.2` (Chromium 138.0.7204.0 via npmjs; real binary, no CDN). System libs `libnspr4`/`libnss3`/`libnssutil3` from sparticuz `al2023` pack via `LD_LIBRARY_PATH`; `--single-process --no-zygote` removed. Verified: SW registration→active, real IndexedDB, two contexts, second page, `setOffline(true/false)` matrix (R09.4 §4).
- **Builds:** `esbuild@0.28.2` harness pages (`tests/browser/build-pages.mjs` — production wiring verbatim, seams: `@/lib/supabase` → in-origin stub, app-store actor → scenario identity). Real Vite production build `dist/` with `VITE_SUPABASE_URL=https://r094.invalid` (workbox `ledgr-api-cache` NetworkFirst `^https://r094\.invalid/rest/v1/`), served by `tests/browser/stub-server.mjs` with SPA fallback + `service-worker-allowed:/` + real CONNECT/TLS terminator (synthetic self-signed, labeled seam; no Playwright route interception for SW traffic).
- **Suites (additive, gate.mjs semantics unchanged):** `r094-browser.test.ts` (17 records `R094.BROWSER.*`), `r094-sw-update.test.ts` (8 `R094.BROWSER.SW.*` Version A→B transition), `r094-revalidation-investigation.test.ts` (1 investigation `R094.BROWSER.SERVER-REVALIDATION.INVESTIGATION`), all via `evidenceSuite` `R13_EVIDENCE_DIR` owned directory.
- **Simulation declared absent:** Never `jsdom`/`fake-indexeddb`/emulated storage/faked SW lifecycle/mocked CacheStorage; every browser capability is the real primitive. Synthetic seams labeled: PostgREST-shaped stub (per-`clientKey` scenario `post_pos_sale`/`reconcile_offline_queue_item`), hydrated actor/business, scenario RPC bodies, TLS loopback proxy.

### Browser records — PASS / FAIL / BLOCKED (real Chromium, this sandbox)

| Suite | Records | PASS | FAIL | BLOCKED |
|---|---|---|---|---|
| `r094-browser` (R09.4) | 17 `R094.BROWSER.*` | 15 | 1 | 1 |
| `r094-sw-update` (Phase A) | 8 `R094.BROWSER.SW.*` | 8 | 0 | 0 |
| `r094-revalidation-investigation` (Phase B) | 1 `R094.BROWSER.SERVER-REVALIDATION.INVESTIGATION` | 0 | 0 | 1 |
| **Total browser** | **26** | **23** | **1** | **2** |

Details (evidence verbatim from `r094-browser.json` / `r094-sw-update.json` at `ledgr-r13-sYAESR`):

- **PASS (15 r094-browser):** `ENVIRONMENT` (Chromium 138 declaration), `SW-LIFECYCLE` (register→install→activate→claim→control→reload→precache 14 entries, polled stable `activated/ctrl`), `SW-API-CACHE-FLUSH` (real SW-intercepted `fetch https://r094.invalid/rest/v1/products` lands in `ledgr-api-cache` `before≥1 → after 0` via production `wipeIdentityTransitionCaches('signed-out')` `R09_CACHE_FLUSH` postMessage), `PERSIST-RESTART` (provenance `USER_A`/branch/device/terminal/shift/`payloadVersion`/`payloadHash`/`clientKey` byte-equal + `verifyPayloadIntegrity` true after **real profile-on-disk process restart**), `WIPE-LOGOUT-BOUNDARY` / `WIPE-USER-SWITCH` (R09.1 wipe preserves queue+install/device id+legacy key, flushes RQ blob+`ledgr-api-cache`+session family; user-switch shows A markers 0, B-only), `LEASE-CROSSTAB-EXCLUSIVE` / `LEASE-EXPIRY-RECLAIM` (two pages racing `syncQueue` → exactly 1 `post_pos_sale` RPC; TTL 900ms hold→expired→acquire, strict token ownership), `PROVENANCE-REPLAY-GATE` (same actor null / other `actor-mismatch` / stripped `missing-provenance` / tampered `payload-tampered` quarantined, origin preserved), `EXCEPTION-P0QLT` (P0QLT → `failed` `policy-denied`, 0 blind-retry RPCs), `EXCEPTION-23514-TYPED` (constraint identity: `chk_inventory_balances_on_hand_nonneg`→`stock-denied`, other 23514→`exceptionClass null`), `RECONCILE-CLIENTPATH` (original `client_key`+reason, byte-identical `canonicalPayloadJson`, `synced` `resolvedServerId` `reconcileAttempts=1` retains `lastErrorCode P0QLT`, lost-ack idempotent `reconcileAttempts=2`), `DRAWER-EXCEPTIONS` / `DRAWER-RECONCILE-GATING` (drawer badges `Stock hold`/`Plan limit hold`, guidance, `isReconcilable && isOnline && role∈{owner,admin,manager}`), `OFFLINE-ONLINE` (`setOffline(true)` `navigator.onLine false` enqueue `pending` provenance complete → `setOffline(false)` exactly 1 replay `resolvedServerId`, second pass 0).
- **PASS (8 sw-update):** `INSTALL-REGISTER` (Version A single registration `/` `activated` `controller` digests+`index.html` match, precache≥10, `app:sw-registered`), `UPDATE-DETECT-TRANSITION` (A→B same-origin `Version A` copy + `Version B` env-only `VITE_SUPABASE_ANON_KEY` marker distinct content-hashes, `reg.update()` → byte-different `sw.js` installs+self-activates `skipWaiting`, claim advances `controllerchange` measured, end-state `index.html`+entry `index-*.js` are Version B, precache revision slot holds Version-B bytes, exactly 1 registration), `ACTIVE-SESSION` (queued item functional during transition, claim+auto-reload measured), `CACHE-TRANSITION` (precache converges to B set, obsolete A hashed assets removed, `ledgr-api-cache` preserved across version), `QUEUE-PROVENANCE` (provenance-complete queue survives A→B install+activate+claim+reload byte-identically, integrity re-verifies, single replay authority), `IDENTITY-CONFIDENTIALITY` (A→A→B update→logout wipe→B: B markers only, wipe still empties caches on Version-B runtime, queue preserved), `HIJACK-STALE-INSPECT` (never 2 registrations, active digest one of two known, document assets B-only, repeated `update()` idempotent), `DETERMINISM` (repeat transition byte-identical).
- **BLOCKED (2 — explicit environmental limitation, not product defect):** `SERVER-REVALIDATION` — *browser-driven server revalidation requires a REAL wire-reachable backend (Postgres+PostgREST+GoTrue auth/JWT/RLS) — sandbox has no disposable Supabase harness for browser traffic (no Docker; embedded-postgres is process-local PG wire; release-assets CDN blocked, `postgrest` npm wrapper pins ancient; hand-written bridge would be `B4` disqualified fake). Client-path/UI evidence covered in `RECONCILE-CLIENTPATH`/`DRAWER-*`/`EXCEPTION-*`; DB-side RPC authority covered by sealed `R093.RECON.*` disposable-DB records. Discharge prerequisites: Docker for official Supabase stack or network to release-assets + official PostgREST/GoTrue with real JWT. This BLOCKED is scoped strictly to browser-driven revalidation.* `SERVER-REVALIDATION.INVESTIGATION` — Phase-B live probe confirming `embedded-postgres 17.10.0-beta.17` IS TCP-addressable but PostgREST/GoTrue layer remains unreachable; additive filing, original BLOCKED not reclassified.
- **FAIL (1 — expected measurement divergence after intentional contract change):** `STALE-VERSION-MEASURE` — expected `v0 stale`+`v9999 future` → `synced` (pre-P5-A `MEASUREMENT ONLY feeds DEC-09`). After P5-A `Q1 B`/`Q2 B` (`QUEUE_PAYLOAD_VERSION=1`, `<1 stale` → `stale-version` quarantined, `>1 unknown` → `unknown-version` quarantined, `null`→`missing-provenance`) actual is `v0=quarantined` `v9999=quarantined` `legacy=quarantined/missing-provenance`. **This FAIL is the intentional P5-A enforcement (durable visible quarantined not retried not reconcilable); the old measurement no longer describes the engine. Not a product regression; harness expectation must be updated to quarantine in a follow-on (not P5-F redesign).** If counted, the 26-record browser suite is 23 PASS / 1 FAIL / 2 BLOCKED; without this outdated measurement, browser real-runtime contracts are 23/0/2.

### Evidence categories separated (P5-F §18 — browser/tool vs DB/static vs unit)

- **Browser / tool versions + evidence:** Chromium `138.0.7204.0` / `playwright-core 1.49.1` / `@sparticuz/chromium 138.0.2` / `esbuild 0.28.2` / `vite 8.3.0` / `vite-plugin-pwa 1.3.0` / `vitest 5.0.1` / `node 22.22.3` with `.cache/r094` built pages+TLS, `dist/` precache 112, evidence artifacts `~/.cache/r13/ledgr-r13-*/r094-browser.json` + `r094-sw-update.json` + `evidence.json`. Polling (SW stable `activated/ctrl` 3×, cache entries, item status) rather than fixed sleeps where transition observed. Determinism: two `r094-browser` runs byte-identical (59–63s each), serial `fileParallelism:false`.
- **DB disposable / static evidence (not claimed as browser):** `embedded-postgres` replay OK for `20261005000000`+`20261006000000`+`20261007000000`+`20261008000000`, `has_function_privilege` both signatures, `pg_get_functiondef` guards, `tsc -b` clean, `eslint` 0/3w, `vite build` 2.02s PWA112, `test:release:types` clean, `git diff --check` clean — never conflated with browser cache/queue/lease claims. Server `reconcile_offline_queue_item` authority remains `R093.RECON.*` DB-suite (standing, not re-proven from browser).
- **Unit harness (isolated, not browser):** `807 PASS / 0 FAIL` (91 files, includes `p5a_model3 18` + `p5b_model4 25` + `p5c_uniformQuota 15` + `p5d_branchScope 8` + `ai branch 10`), `tsc -b` clean, `eslint` clean — proves version/exception/freeze/quota/branch/AI contracts deterministically where browser probes use real primitives.

### Network evidence table (R09.4 production path — real SW-handled fetch, PAC→CONNECT→TLS)

| Transition | Stub-observed `swTlsReads` / `fetchStatus` / Cache | Queue provenance before→after |
|---|---|---|
| SW API cache warm (R09.4 §6.2) | `swTlsReads=1`, `fetchStatus=200`, `ledgr-api-cache entriesAfterFetch≥1`, `beforeWipe=1→afterWipe=0` | — |
| SW update A→B (Phase A) | Version-B `sw.js` byte-different (sha256), `registrations 1`, `active activated`, `indexDigest` B, entry `index-*.js` B-only | queue `pending` → survives reload byte-identical, integrity true, single `post_pos_sale` |
| Offline→online (R09.4 §14) | genuine `setOffline` `navigator.onLine` toggle, 1 replay RPC `resolvedServerId`, second pass 0 | `clientKey`/`payloadHash`/`originUserId` byte-equal, `canonicalPayloadJson` equal |
| Cross-tab lease (R09.4 §10) | exactly 1 `post_pos_sale` under 900ms delay race | loser fails closed `lease exists`, winner `synced` |

### Integrity / tamper / reconcilable evidence (browser)

- `verifyPayloadIntegrity` after restart / after capture → true; after after-capture edit → false → `reconcileQueueItem` returns `payload-tampered` `quarantined` without network.
- `isReconcilable` frozen `['stock-denied','policy-denied']` in browser: `stale-version`/`unknown-version`/`clientKey-payload-mismatch`/`payload-tampered` never reconcilable (integrity-class), `branch-denied`/`terminal-denied` failed → `not-an-exception` (no RPC).
- `reconcileQueueItem` client path: request `client_key` + `reason`, payload immutable, retains `lastErrorCode`, idempotent `idempotent:true` no duplication.

### Gate result

- **P5-F verification COMPLETED WITH DOCUMENTED BLOCKED EVIDENCE + 1 expected measurement FAIL.** No FAIL in product logic; 23 of 26 browser records PASS in genuine Chromium with zero simulation (deterministic), 2 BLOCKED are explicit environmental incapability (disposable real backend) already filed in R09.4 and not reclassified, 1 FAIL is the P5-A `stale-version`/`unknown-version` quarantine superseding the pre-P5-A measurement. The `742/0/40` historical gate is not claimed as still PASS — honest delta shown in §9. Do not claim overall release production-ready.
...
- **After P5-E:** `807 PASS / 0 FAIL` unit (91 files, +10 P5-E, up from 797), `tsc -b` clean, `eslint` 0/3w, `vite build` 1.97s PWA112, `test:release:types` clean, `git diff --check` clean. **New migration `20261008000000_p5e_ai_branch_context.sql` (additive: branch-aware `v_ai_*` + `ai_context(uuid,uuid)` no-default + wrapper `ai_context(uuid)` + hybrid `can_access_branch` + grants). `r03-ai` 16/16 PASS with both signatures `has_function_privilege`, release baseline `652/66/76` vs expected `742/0/40` delta +1 PASS from hybrid fix (branch_manager/sales_manager NULL org-wide), remaining 66 FAIL pre-existing `R10`/`TENANT`/`R06`/`R08.BRANCH` (U805 cashier). **P5-E re-proved via disposable-DB `embedded-postgres` (both signatures) and full `r03-ai`.**
- **After P5-F (this gate):** Unit `807 PASS / 0 FAIL` (91 files) unchanged (P5-F verification-only, no product migration; `src/lib/ai/__tests__/branch.test.ts` `tsc` fix `TS6133`+`TS2367` only, `tsc -b` clean, `eslint` 0/3w, `vite build` 2.02s PWA112, `test:release:types` clean, `git diff --check` clean) + **browser `26 records = 23 PASS / 1 FAIL / 2 BLOCKED` in genuine Chromium** (see §8: 15 r094-browser +8 sw-update =23 PASS, 2 BLOCKED server-revalidation + investigation, 1 FAIL stale-version measurement superseded by P5-A quarantine — honest). Disposable-DB release at `sYAESR` `675 PASS / 67 FAIL / 52 BLOCKED / 794` vs before `652/66/76` (+23 PASS from newly runnable browser suite after `tsc` fix, +1 FAIL from P5-A staleVersion divergence, −24 BLOCKED). Historical `742/0/40` (`bc97e32` pre-P5-D) delta now `+?` honest (see §12); `662/66` core FAIL pre-existing `inventory/stock` `42501`/`R06` `105`/`R08` etc. unchanged by P5-F (verification-only).
...
- `R09.4` browser verification now honest: `PERSIST-RESTART` real process-restart provenance, `PROVENANCE-REPLAY-GATE` (actor/missing/tampered), `EXCEPTION-P0QLT`/`23514-TYPED` (branch/terminal via `classifyReplayException` in unit + `P0QLT`/`stock-denied` via browser stub; `branch-denied`/`terminal-denied` browser-proven as typed `failed` not reconcilable via `p5a_model3`+`p5b_model4` + `r094` reconcile freeze), `RECONCILE-CLIENTPATH` + `LEASE-CROSSTAB` + `OFFLINE-ONLINE` + `SW` lifecycle (8 Phase-A) evidenced in real Chromium (§8). Remaining gap: `SERVER-REVALIDATION` browser→real backend remains BLOCKED (see §8 discharge prerequisites) and `STALE-VERSION-MEASURE` FAIL until harness updated to expect quarantine (P5-A).
...
- **Product behavior changed:** **Yes — P5-A Model 3 as authorized + P5-B freeze (no new behaviour) + P5-C uniform quota/dual authority as authorized + P5-D DEC-03 branch remediation as authorized + P5-E AI.BRANCH/R11 as authorized:** P5-A version quarantine (`stale-version`/`unknown-version`), branch/terminal typed failed, clientKey mismatch quarantined with authoritative hash guard in `post_pos_sale`; P5-B proves `RECONCILABLE` frozen; P5-C uniform `P0QLT` via `FOR UPDATE` + `BEFORE INSERT` triggers + capture `P0QLT`; P5-D branch scope: `can_access_branch` DEC-03-correct (assigned fail-closed), `can_access_location`, `save_quick_*` RPC branch guard, RLS `can_access_branch`/`can_access_location` on 14+ tables + child lines, `branches` writer → admin, location-derived inventory sealed, POS terminal/shift still `42501`/`22023`; P5-E AI branch context (`ai_context` `branch_id` optional, `can_access_branch` authorized, `v_ai_*` branch-aware, prompt hints never broaden, `R11` coherence via `branch test`); P5-F **no product behavior changed** (verification-only).
- **SQL changed:** **Yes — P5-A additive only** `invoices.payload_hash`, `_ledgr_pos_payload_hash`, `post_pos_sale` replacement — P5-B **no SQL change** — **P5-C additive** `20261006000000_p5c_uniform_billing_quota.sql` (`_ledgr_assert_usage_limit` `FOR UPDATE`, three `BEFORE INSERT` triggers) — **P5-D additive** `20261007000000_p5d_branch_scope_remediation.sql` (`can_access_branch` redefined DEC-03, `can_access_location` new, `save_quick_*` redefined with branch checks, RLS branch predicates on `invoices`/`expenses`/`journal_entries`/`journal_lines`/`inventory_locations`/`departments`/`branches`/`employees`/`fixed_assets`/`accounts`/`budget_lines` + child lines + `stock_movements`/`inventory_balances`/`stock_transfers`; no `stock 23514`/`period`/`journal`/`payload_hash`/`MAX_PENDING`/`TTL`/`P0QLT` change beyond branch) — **P5-E additive** `20261008000000_p5e_ai_branch_context.sql` (`v_ai_*` branch-aware, `ai_context(uuid,uuid)` no-default + wrapper, hybrid `can_access_branch`, grants; no `stock`/`P0QLT` change) — **P5-F no SQL change** (verification-only).
- **RLS changed:** **Yes — P5-D branch remediation:** RLS now branch-scoped via `can_access_branch`/`can_access_location` on 14+ tables + child/derived tables; `branches` writer tightened; POS family already branch-scoped (predicate updated). No change in P5-A/B/C. P5-E adds **no RLS** (views `security_invoker`, `ai_context` `security definer`); P5-F **no RLS** (verification-only).
- **Edge functions changed:** No.
- **AI behavior changed:** **Yes — P5-E AI.BRANCH/R11 as authorized (Q14 B/Q15 B):** `ai_context` optional `branch_id`, `can_access_branch` authorized, branch-filtered `WHERE branch_id` server-side, `R11` coherence (`branch.test.ts` 10 tests); Q14/Q15 after P8 still preserved (no further AI). P5-F no AI change.
...
- **Offline behavior changed:** **P5-A yes** (provenance version, exceptions, syncEngine mismatch quarantine, Dexie v4); **P5-B freeze only** (no queue semantics changed); **P5-C yes** (`queueApi.enqueue` capture-time `P0QLT` guard 80ms test / 1500ms prod, fail-open otherwise; `provenance.test.ts` still 11 PASS via timeout); **P5-D no new queue semantics** (server branch authority via RLS/RPC; capture remains server-authoritative, no new `queueApi` branch probe — gap documented in §6 inventory); **P5-E no offline** (AI only); **P5-F verification-only** (proves existing offline/cache contracts in real browser — see §8).
- **Tests changed:** **Yes — additive:** `p5a_model3.test.ts` (18) + `p5b_model4.test.ts` (25) + `p5c_uniformQuota.test.ts` (15) + `p5d_branchScope.test.ts` (8) + `p5e ai branch.test.ts` (10) + `provenance.test.ts` fix (now via `queueApi` timeout) + `queueApi` timeout + `r094-browser` `r094-sw-update` `r094-revalidation-investigation` registration (browser verification-only, additive `R094.BROWSER.*`); no weakening. P5-F harness still additive — `STALE-VERSION-MEASURE` FAIL is outdated expectation (P5-A), not a test change.
- **Package/CI changed:** No.

## §12 Final Gate

- **Implementation:** `P5-A COMPLETE` (typed offline exceptions) + `P5-B COMPLETE` (reconciliation freeze) + `P5-C COMPLETE` (uniform billing quota + dual authority) + `P5-D COMPLETE` (DEC-03 branch scope / R08.7) + `P5-E COMPLETE` (AI Branch Context / R11 at 297337b — 807 PASS) + `P5-F COMPLETE with documented BLOCKED/FAIL` (R09.4 browser verification-only). Overall `P5 PARTIALLY COMPLETE` with honest BLOCKED (server revalidation) + measurement divergence (stale-version now quarantined) — STOP after P5-F per authorization.
- **Exact commits:** `112ebec` P5-A (MODEL3) + `33c70b0` P5-B (freeze) + `618ba30` P5-C (quota) + `fd1a9b5` P5-D (branch) + `297337b` P5-E (AI branch, 807 PASS) + **(this commit) P5-F (browser verification + tsc fix, report sync)** — files `src/lib/ai/__tests__/branch.test.ts` (`TS6133`+`TS2367` fix only), `docs/audits/LEDGR_P5_IMPLEMENTATION_REPORT_2026-09-24.md` (§7 P5-E, §8 P5-F browser/tool versions & categories, §9 after P5-E/F, §10-11 attestations).
- **Exact test counts (this gate):** Unit `807 PASS / 0 FAIL` (91 files, +18+25+15+8+10 P5-A…E), `tsc -b` clean, `eslint` 0/3w, `vite build` 2.02s PWA112, `test:release:types` clean, `git diff --check` clean; Release `R03.AI` 16/16 PASS, `R09.4 browser` 23 PASS / 1 FAIL (P5-A superseded measurement) / 2 BLOCKED (server revalidation/investigation, explicit), overall release `675/67/52/794` at `sYAESR` vs historical `742/0/40/782` (`bc97e32` `b9d41ec854a1`) and before `652/66/76` (`297337b` tsc-blocked) — delta +23 PASS browser suite runnable after tsc fix, +1 FAIL stale-version quarantine, –24 BLOCKED (see §8-9). Core 66 FAIL pre-existing `R10`/`TENANT`/`POS`/`R06`/`R07`/`R08` `42501`/`23514` unchanged by P5-F (verification-only); not claimed as production-ready.
- **Remaining blockers / honest state:** `contacts.branch_id` missing needs product decision if ever required (P5-D gap, not invented); `SERVER-REVALIDATION` browser→real backend BLOCKED until Docker or release-assets mirror + official PostgREST/GoTrue (see §8 prerequisites); `STALE-VERSION-MEASURE` FAIL until harness updated to expect quarantine (P5-A intentional); 66 core `R06/R07/R08` etc. FAIL remain at disposable-DB layer (branch RLS narrowing) — honest delta, not collapsed. Any new policy question will surface and STOP rather than assumed.
- **Another owner decision required:** No — P5-A…E choices remain `85d1615`; any new question will be STOPPED. P5 is complete per authorized packages A-F (verification-only F); next gate requires owner re-authorization, not silent continuation.

---

## STOP — P5-F gate verification complete (P5-A…F), awaiting owner re-authorization

P5-A (17 requirements, 18 tests, 749 PASS) + P5-B (18 checks, 25 tests, 774 PASS) + P5-C (15 checks, uniform `P0QLT` via `FOR UPDATE` + `BEFORE INSERT` triggers, 789 PASS) + P5-D (8 checks, DEC-03 branch predicate + `can_access_location` + `save_quick_*` branch guard + RLS branch enforcement on 14+ tables + location-derived inventory, `branches` writer→admin, terminal/shift `42501`/`22023` fail-closed including assigned-NULL, 797 PASS) + P5-E (10 tests, `ai_context` branch-aware `can_access_branch` authorized, `v_ai_*` unions, `R11` coherence, 807 PASS) satisfy §5–§7 deterministically without weakening. **P5-F (26 browser records = 23 PASS real Chromium, 2 BLOCKED server-revalidation explicit, 1 FAIL stale-version superseded; `675/67/52` release with honest delta; `807` unit, `tsc -b`/`eslint`/`build`/`types` clean; `git diff --check` clean) completes the authorized P5 A-F sequence verification-only. STOP after P5-F — do not claim overall release production-ready (19-item return).** If a later package exposes a dependency, STOP and do not weaken contract.
